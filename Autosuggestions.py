
from selenium.webdriver import Keys
from selenium import webdriver
from selenium.webdriver.chrome.service import Service
from selenium.webdriver.common.by import By
from selenium.webdriver.support.ui import Select

import time

s = Service("C:\\Users\\nagua\\PycharmProjects\\SeleniumExamples\\drivers\\chromedriver")
driver = webdriver.Chrome(service=s)
driver.maximize_window()

driver.get("https://www.bing.com/")

# p1=driver.find_elements(By.TAG_NAME, "q")
driver.find_element(By.NAME, "q").send_keys("How")
p1 = driver.find_elements(By.XPATH, "//*[@id='sb_form_q']")

time.sleep(3)

print(len(p1))

# to print the values in the dropdown
for element in range(len(p1)):
    if p1[element].is_displayed():
        print(p1[element].text)
#below command for screenshot
driver.get_screenshot_as_file("C:/Users/nagua/Desktop/SEwithPY/Test.png")
time.sleep(3)
driver.quit()
